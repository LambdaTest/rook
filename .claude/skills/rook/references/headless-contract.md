...
| `rook profile test [id] [--goal <text>] [--yes] [--allow <rule>]`                                                                                                                                                                                                                       | yes                                                              | call the agent once                                                              |
...
Help quirk: `rook profile add --help` prints the root help. `rook help profile`
prints the subcommands with their flags. Use `rook help <group>`.
...
`remedy` is present when the admission gate refused: `login`, `topup`,
`request_access`, `new_session`, or `report` (a bug; run `rook export logs`).
...
- stdout: the JSON document under `--json`; otherwise the human output.
...
One line per event on stderr, rendered by rook, not JSON: `phase_start`,
