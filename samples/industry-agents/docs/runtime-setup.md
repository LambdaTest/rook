...
Open **http://127.0.0.1:4310**, or the address printed in the terminal. Keep it running. To check your key separately, use `npm run check:llm`; this makes a model request.
...
The [interactive guide](testing-with-rook.md) covers service conditions, MCP and repair comparisons. For the supplied 18-case pack, run `npm run rook:setup` and `npm run rook` from the selected edition's folder.
...
For another provider, a separate workspace or connection changes, see [integration details](rook-integration.md).
