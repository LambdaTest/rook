# Juniper Goods — QE runtime setup
...
Open **http://127.0.0.1:4317**, or the address printed in the terminal. Keep it running. To check your key separately, use `npm run check:llm`; this makes a model request.
...
```bash
npm run rook:prepare -- customer-support-agent /tmp/customer-support-agent
cd /tmp/customer-support-agent
rook login
