...
Target: Claims & Coverage Assistant. Base URL: http://127.0.0.1:4315. Configure this application as described in README.md, using MODEL_API_KEY through .env or the OS environment, then start the Node 22+ service with `npm run demo -- insurance-agent` from the collection root (`samples/industry-agents`).
...
```bash
curl -sS http://127.0.0.1:4315/api/sessions -H 'Content-Type: application/json' -d '{"variant":"vulnerable","fault":"none"}'
```
