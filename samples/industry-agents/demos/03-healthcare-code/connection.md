...
Target: Patient Access Assistant. Base URL: http://127.0.0.1:4312. Configure this application as described in README.md, using MODEL_API_KEY through .env or the OS environment, then start the Node 22+ service with `npm run demo -- healthcare-agent-code` from the collection root (`samples/industry-agents`).

The vulnerable variant deliberately omits the documented business checks. The hardened variant implements the demonstrated repair: Check patient ownership and slot capacity in the tool; route urgent symptoms to a human. These are fictional service rules, not medical guidance. PRD.md remains the intended behavior for both variants; do not derive expected success from baseline defects.
...
```bash
curl -sS http://127.0.0.1:4312/api/sessions -H 'Content-Type: application/json' -d '{"variant":"vulnerable","fault":"none"}'
```
