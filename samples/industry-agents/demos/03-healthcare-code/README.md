...
Open **http://127.0.0.1:4312**, or the address printed in the terminal. Keep the app running. For help with keys or startup, see [runtime setup](runtime-setup.md).
...
```bash
npm run rook:prepare -- healthcare-agent-code /tmp/healthcare-agent-code
cd /tmp/healthcare-agent-code
rook login
...
```bash
npm run setup -- healthcare-agent-code
ROOK_ENV=prod npm run rook:ci -- healthcare-agent-code --project PROJECT_ID
```
